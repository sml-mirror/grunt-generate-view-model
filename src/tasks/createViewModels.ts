...
export function createMetadatas(properties: any) {
      ...
      for (var file of files){
          ...
          var stringFile = fs.readFileSync(file.src, "utf-8");
 ...
 export function  CreateFiles(metadata: FileMetadata[]) {
      ...
      for ( var i = 0; i < metadata.length; i++ ) {
          ...
          if (c && c.trim()) {
            ...
            mkdirp(getDirName(metadata[i].filename), function () {var f; });
            fs.writeFileSync(metadata[i].filename, c, "utf-8");
           /* fs.mkdirSync(metadata[i].filename);
            fs.openSync(metadata[i].filename, "w");*/
          //  fs.writeFileSync("C:/Git/AbatapCompany/grunt-generate-view-model/test/dist/src/tasks/_autogenerated_views/t.ts", c, "utf-8");
